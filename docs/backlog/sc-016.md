...
# [SC-016] ✅ DONE - Modernize WSL Manager TUI with PwshSpectreConsole
...
- [x] Every interactive step offers an explicit way back to the main menu (**Back to main menu** picker entry, or answering No to a confirmation); Ctrl+C is not the mechanism because `cmd.exe` intercepts it under the `.bat` wrapper (see SC-016d)
- [x] Distro table rendered with `Format-SpectreTable` (borders, colored status)
...
- [x] Existing command functions in `commands.ps1` are not structurally changed
- [x] All existing tests continue to pass
- [x] New TUI functions have Pester unit tests with mocked Spectre cmdlets
