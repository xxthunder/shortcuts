...
# [SC-040] ✅ DONE - Add Windows-side px authenticating proxy tool
...
- [x] `px-proxy.ps1` pauses for a key press before exiting (`-WaitForKey`, default `$true`) so a Keypirinha/`.bat`-launched console stays open until the user reads the result; the pause fires on both success and failure and is skipped in CI/test environments; pass `-WaitForKey:$false` for unattended use (added 2026-07-16; unit-verified)
...
- [x] `docs/runbooks/px-proxy.md` simplified for a general audience (reopened 2026-07-17): plain language over jargon, Prerequisites reassures "you probably have everything already", and the WSL-reachability caveat is dropped because this repo's wsl-manager (`Invoke-ConfigureWsl`) already sets `networkingMode=mirrored`
