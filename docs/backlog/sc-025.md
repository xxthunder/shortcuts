...
**Acceptance Criteria**:
- [x] `Install-WslDocker` adds `[automount] options = "metadata,umask=022"` to wsl.conf when not already present
- [x] `Install-WslPodman` adds `[automount] options = "metadata,umask=022"` to wsl.conf when not already present
- [x] Existing user-defined `[automount]` options are preserved (not overwritten)
- [x] Unit tests cover the new automount section in both docker and podman flows
- [x] All existing tests continue to pass
- [x] Extract `Set-WslConf`, `Test-WslSystemdConfigured`, `Test-WslInteropConfigured`, `Test-WslAutomountConfigured` from `user.ps1` into dedicated `wsl-conf.ps1`
- [x] Tests for extracted functions moved to `wsl-conf.Tests.ps1`
