...
# Managed by wsl-manager (SC-042/SC-043). Regenerated by setup-proxy; do not edit.
...
# Corporate TLS inspection: point tools that ship their own CA store (uv, Node,
# pip/requests, Go, ...) at the system trust store, which holds the corporate
# root CA (SC-043). Guarded so a missing bundle can't break TLS.
