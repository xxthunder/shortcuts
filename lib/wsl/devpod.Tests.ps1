...
Describe "Invoke-WslSyncSshConfig" {
    ...
    Context "Prerequisite validation - DevPod not installed" {
        It "Should throw when DevPod is not installed" {
            ...
            { Invoke-WslSyncSshConfig -DistroName "Debian" -Confirm:$false } | Should -Throw "*DevPod*not installed*"
        }
        ...
        It "Should suggest setup-devpod in error message" {
            ...
            { Invoke-WslSyncSshConfig -DistroName "Debian" -Confirm:$false } | Should -Throw "*setup-devpod*"
        }
    }
